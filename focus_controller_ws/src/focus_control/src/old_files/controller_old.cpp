...
int main(int argc, char **argv){
	...
	outFile_car.open("/home/acostley/Desktop/focus_out/output_data_car.txt");
...
float longitudinalPID(){
	...
	bool flag = false;
	float limit = 20;
	float decel_time = 2; //2 seconds to get to desired speed
	float Kp_bpp = 0.2;
	float Ki_bpp = 1.2;
	float limit_bpp = 2.5; //2.5 m/s^2 is the limit of passenger comfort
	...
	if(error < 0){
		...
		pid_out = discrete_lateralPID(decel,0,flag,Kp_bpp,Ki_bpp,limit_bpp,Ts);
